<script>
  ...
  $(document).ready(function () {
    $('.datatable').DataTable({
        ajax: '{{ url("admin/internal-storage/getAllDatatable") }}',
        serverSide: false,
        processing: true,
        deferRender: true,
        type: 'GET',
  ...
  $('#addForm').submit(function(e) {
    ...
    $.ajax({
      url: '{{ url("admin/internal-storage") }}',
      method: 'post',
      data: fd,
      cache: false,
      contentType: false,
  ...
  $(document).on('click', '.btn-edit', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
          ...
          $('#editModal #output-img').attr('src', '/storage/images/pc-components/internal-storage/' + response.data.image);
          $('#editModal #name').val(response.data.name);
          $('#editModal #price').val(response.data.price);
          $('#editModal #url').val(response.data.url);
          $('#editModal #capacity').val(response.data.capacity);
